...
// Also wakes and sleeps on rising edge of button press
...
// Globals (Time in UTC)
// ++++++++++++++++++++ Change me ++++++++++++++++++
uint8_t  WakeUp_StartHour       = 12;  // Hour in 24 hour clock wake: 5:35am
uint8_t  WakeUp_StartMinute     = 35;  // Minutes 
uint8_t  Bedtime_SleepHour      = 01;  // Hour in 24 hour clock sleep: 6:10pm
uint8_t  Bedtime_SleepMinute    = 10;  // Minutes 
...
void loop() 
{
    ...
    if(pi_awake == false)
    {
        ...
        attachInterrupt(1, alarm_isr, RISING);   // button pin to wake on button press
        ...
        // +++++ Power the Rpi ++++++
        //only power up from March 1 to Sept 30
     ...
     else
     {
          ...
          // Allow wake up alarm to trigger interrupt on falling edge.
          attachInterrupt(0, alarm_isr, FALLING);   // Alarm pin
          attachInterrupt(1, alarm_isr, RISING);   // button pin to sleep on button press
          SleepyPi.enableWakeupAlarm(true);
